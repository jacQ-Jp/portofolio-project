    ...
    <section class="container mx-auto px-8 py-16">
        <div class="max-w-xl mx-auto">
            <div class="flex items-center justify-between gap-4">
                <h1 class="text-2xl font-semibold tracking-[0.18em] uppercase text-gray-800">Add Skill</h1>
            ...
            <form id="skillForm" method="POST" action="{{ route('skills.store') }}" class="bg-white/80 backdrop-blur rounded-xl border border-black/5 shadow-[0_20px_60px_rgba(0,0,0,0.08)] p-8 space-y-5">
                @csrf
                ...
                <div>
                    ...
                    <input id="level" name="level" type="number" value="{{ old('level', 50) }}" required min="1" max="100"
                           class="mt-2 w-full rounded-lg border border-black/10 bg-white px-4 py-3 text-sm outline-none focus:border-gray-400 focus:ring-2 focus:ring-gray-200" />
                ...
                <div>
                    ...
                    <input id="category" name="category" type="text" value="{{ old('category') }}" maxlength="50"
                           class="mt-2 w-full rounded-lg border border-black/10 bg-white px-4 py-3 text-sm outline-none focus:border-gray-400 focus:ring-2 focus:ring-gray-200" />
                </div>

                <div>
                    <label class="block text-xs font-semibold tracking-[0.2em] uppercase text-gray-600" for="sort_order">Sort Order</label>
                    <input id="sort_order" name="sort_order" type="number" value="{{ old('sort_order', 0) }}" min="0" max="65535"
                           class="mt-2 w-full rounded-lg border border-black/10 bg-white px-4 py-3 text-sm outline-none focus:border-gray-400 focus:ring-2 focus:ring-gray-200" />
                </div>

                <button type="submit" class="w-full btn-primary !rounded-lg !py-3">Save</button>
            </form>
        </div>
    </section>
