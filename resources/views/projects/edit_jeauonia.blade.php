...
<div class="container mx-auto px-8 py-16">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="text-center mb-12">
            <h1 class="section-title">EDIT DESIGN</h1>
            <p class="text-gray-600 font-light text-sm tracking-widest">UPDATE YOUR DESIGN</p>
        </div>
        ...
        <!-- Form -->
        <form action="{{ route('projects.update', $project) }}" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded border border-gray-200">
            @csrf
            @method('PUT')
            ...
            <div class="mb-8">
                ...
                <textarea id="description" name="description" rows="5" placeholder="Tell the story of your design..." 
                          class="w-full px-4 py-3 border border-gray-300 rounded focus:outline-none focus:border-gray-500 font-light text-sm @error('description') border-red-500 @enderror">{{ old('description', $project->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-2 font-light">{{ $message }}</p>
                @enderror
            </div>
            ...
            @if($project->media_path)
                <div class="mb-8 p-6 bg-gray-50 rounded border border-gray-200">
                    <h3 class="font-light text-gray-700 mb-4 tracking-widest text-xs font-semibold">CURRENT MEDIA:</h3>
                    <div class="rounded overflow-hidden border border-gray-200">
                        @if($project->media_type === 'image')
                            <img src="{{ asset('storage/' . $project->media_path) }}" alt="{{ $project->title }}" class="max-h-64 w-full object-cover">
                        @else
                            <video class="max-h-64 w-full object-cover" controls>
            ...
            <!-- Media Upload -->
            <div class="mb-8">
                <label for="media" class="block text-xs font-semibold tracking-widest text-gray-700 mb-4">CHANGE MEDIA (Optional)</label>
